...
class XHeaderCtrl : public CHeaderCtrl
{
    DECLARE_DYNCREATE(XHeaderCtrl)
...
public:
	...
	ImpEditCol* GetSelectColumns() { return m_pSelectColumns; }
	void SetSelectColumns(ImpEditCol* pSelectColumns) { m_pSelectColumns = pSelectColumns; }
	void SetCurSel(int nSubItem) { (m_nCurSel < 0) ? m_nCurSel = nSubItem : m_nCurSel = -1; }

	bool GetColumnWriteFlag(int column) const { return m_pSelectColumns->m_szFieldData[column].bMutable; }
	void SetColumnWriteFlag(bool b, int column) { m_pSelectColumns->m_szFieldData[column].bMutable = b;}
...
protected:
	...
	ImpEditCol* m_pSelectColumns;
	int m_nCurSel;

	COLORREF m_cr3DHighLight;
	COLORREF m_cr3DShadow;
	COLORREF m_cr3DFace;
	COLORREF m_crBtnText, m_crBtnSelText;
