...
10	if (tf4) then
     ...
     +         ifit3,syx23,sx23,meanx3)) mstatusmp(imr)=4
	  ...
	  if (tf1.and.(.not.fit1)) mstatusmp(imr)=2
	  if (tf2.and.(.not.fit2)) mstatusmp(imr)=2
	  if (tf3.and.(.not.fit3)) mstatusmp(imr)=2
	 endif
     ...
     +         ifit3,syx23,sx23,meanx3)) mstatusmp(i)=7
	     ...
	     if (tf1.and.(.not.fit1)) mstatusmp(i)=5
	     if (tf2.and.(.not.fit2)) mstatusmp(i)=5
	     if (tf3.and.(.not.fit3)) mstatusmp(i)=5
	    endif
	    if (mstatusmp(i).eq.6) then
	     igd=igd+1
	     xgd1(igd)=logbump(i)
...
c	*******************************************************************************
	...
	implicit none
	integer i,n
	real*4 x,y,av(9),s(9),stvalue,ypl1,ypl2
	integer cyclepoint
